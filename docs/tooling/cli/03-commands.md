...
- Produces module federation assets (frontend) or includes plugin dependencies in the plugin's private `node_modules`, building and `yarn pack`ing the local `workspace:^` dependencies first (backend). 
