    ...
    this.module._resolveFilename = (
      ...
      for (const [realPath, pkg] of scannedPluginManifests.entries()) {
        if (
          [
            searchedPackageName,
            ...(this.options.dynamicPluginPackageNameSuffixes ??
              ['-dynamic'].map(s => `${searchedPackageName}${s}`)),
          ].includes(pkg.name)
        ) {
      ...
      // static package wrapped inside a distinct dynamic plugin package for example.
