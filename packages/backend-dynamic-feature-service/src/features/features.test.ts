...
[
  ...
  it('should access the module federation assets of the frontend plugin through the backend plugin', async () => {
    ...
    expect(await list.json()).toEqual({
      'plugin-test-dynamic': `http://localhost:${server.port()}/api/core.dynamicplugins.frontendRemotes/plugin-test-dynamic/mf-manifest.json`,
    });

    const manifest = await fetch(
      `http://localhost:${server.port()}/api/core.dynamicplugins.frontendRemotes/plugin-test-dynamic/mf-manifest.json`,
    );
