...
describe('postProcessBundlePackageJson', () => {
  it('clears scripts and devDependencies', () => {
    ...
    postProcessBundlePackageJson(pkg, undefined, {}, false);
    expect(pkg.scripts).toEqual({});
    expect(pkg.devDependencies).toEqual({});
    expect(pkg.bundleDependencies).toBeUndefined();
  });

  it('sets bundleDependencies when needsDependencies is true', () => {
    const pkg: Record<string, unknown> = {};
    postProcessBundlePackageJson(pkg, undefined, {}, true);
  ...
  it('merges root resolutions and strips patch prefix ', () => {
    ...
    postProcessBundlePackageJson(pkg, rootResolutions, {}, true);
  ...
  it('does not merge resolutions when needsDependencies is false', () => {
    const pkg: Record<string, unknown> = {};
    postProcessBundlePackageJson(pkg, { dep: '1.0.0' }, {}, false);
...
describe('bundle command', () => {
  ...
  describe('validation', () => {
    it('throws when backstage.role is missing', async () => {
      ...
      await expect(command(defaultOpts)).rejects.toThrow(
  ...
  describe('backend', () => {
    ...
    describe('via createDistWorkspace', () => {
      ...
      it('should produce backend bundle when build=true', async () => {
        await command(defaultOpts);
        const pkg = await fs.readJson(joinPath(ctx.targetDir, 'package.json'));
        expect(pkg.name).toBe(backendPkg.name);
        expect(pkg.bundleDependencies).toBe(true);
        expect(await fs.pathExists(joinPath(ctx.targetDir, '.gitignore'))).toBe(
          true,
        );
        expect(
          await fs.pathExists(joinPath(ctx.targetDir, '.yarnrc.yml')),
        ).toBe(true);
      ...
      it('should pass buildDependencies=false when build=false', async () => {
        await command({ ...defaultOpts, build: false });
        expect(mockCreateDistWorkspace).toHaveBeenCalledWith(
          expect.anything(),
      ...
      it('should assemble local deps into embedded/ and clean up .yarn', async () => {
        ...
        expect(
          await fs.pathExists(
            joinPath(ctx.targetDir, 'embedded', commonRelDir, 'package.json'),
          ),
        ).toBe(true);
        expect(pkg.resolutions['@scope/plugin-foo-common']).toBe(
          `file:./embedded/${commonRelDir}`,
        );
        expect(await fs.pathExists(joinPath(ctx.targetDir, '.yarn'))).toBe(
          false,
        );
      });
    ...
    describe('via createDistWorkspace - failure', () => {
      it('should clean up temp dir and propagate error on createDistWorkspace failure', async () => {
        ...
        await expect(command(defaultOpts)).rejects.toThrow('pack failed');
    ...
    describe('via --pre-packed-dir', () => {
      ...
      it('should copy from pre-packed dir and assemble embedded', async () => {
        ...
        await command({ ...defaultOpts, prePackedDir: prePackedPath });
    ...
    describe('--no-install', () => {
      ...
      it('should skip install and print warning', async () => {
        await command({ ...defaultOpts, install: false });

        expect(
          await fs.pathExists(joinPath(ctx.targetDir, 'package.json')),
        ).toBe(true);
        expect(await fs.pathExists(joinPath(ctx.targetDir, 'yarn.lock'))).toBe(
          true,
        );
        expect(
          await fs.pathExists(joinPath(ctx.targetDir, 'node_modules')),
        ).toBe(false);
        expect(mockRun).toHaveBeenCalledWith(
    ...
    describe('options', () => {
      ...
      it('should remove target when clean=true', async () => {
        ...
        await command({ ...defaultOpts, clean: true });

        expect(
          await fs.pathExists(joinPath(ctx.targetDir, 'existing-file')),
        ).toBe(false);
        expect(
          await fs.pathExists(joinPath(ctx.targetDir, 'package.json')),
        ).toBe(true);
      ...
      it('should write bundle to custom outputDestination with mangled name', async () => {
        ...
        await command({ ...defaultOpts, outputDestination: customOutput });

        const target = joinPath(customOutput, backendMangledName);
        expect(await fs.pathExists(joinPath(target, 'package.json'))).toBe(
          true,
        );
      });
      ...
      it('should write .bundle-output marker to the output directory', async () => {
        ...
        expect(
          await fs.pathExists(joinPath(ctx.targetDir, '.bundle-output')),
        ).toBe(true);
      });

      it('should remove nested dirs that have a .bundle-output marker', async () => {
        // Simulate a previous bundle run that left a marked directory
        ...
        // Override the createDistWorkspace mock to also place the stale
        // directory inside the packed main-plugin output (simulating
        // npm-packlist's basename matching).
        const mainTarget = { name: backendPkg.name, dir: ctx.pluginDir };
        mockCreateDistWorkspace.mockImplementation(async (_pkgNames, opts) => {
          await fs.copy(
            joinPath(fixturesDir, 'dist-workspace', 'backend'),
            opts.targetDir,
          );
          // Inject the stale bundle directory into the packed main plugin
          const mainPluginRelDir = ctx.relDir;
          const nestedDir = joinPath(
            opts.targetDir,
            mainPluginRelDir,
            'old-bundle',
          );
        ...
        expect(await fs.pathExists(joinPath(ctx.targetDir, 'old-bundle'))).toBe(
          false,
        );
      });
    ...
    describe('error handling', () => {
      it('should propagate error and show log when pruneBundleLockfile fails', async () => {
        ...
        mockRun
          ...
          .mockImplementationOnce(
            (
              ...
              waitForExit: async () => {
                const cb = opts?.onStdout ?? opts?.onStderr;
                if (cb) {
                  cb(Buffer.from('yarn prune output line 1\nline 2\n'));
                }
        ...
        await expect(command(defaultOpts)).rejects.toThrow('prune failed');
      ...
      it('should propagate error when installBundleDependencies fails', async () => {
        ...
        await expect(command(defaultOpts)).rejects.toThrow('install failed');
      });
    });
  ...
  describe('frontend', () => {
    ...
    describe('without --pre-packed-dir', () => {
      it('should produce frontend bundle when build=true', async () => {
        ...
        expect(await fs.pathExists(joinPath(ctx.targetDir, 'src'))).toBe(false);
      ...
      it('should keep src/ when "files" explicitly includes it', async () => {
        ...
        setupPackToDirectoryMock();
        await command(defaultOpts);
        expect(await fs.pathExists(joinPath(ctx.targetDir, 'src'))).toBe(true);
      });
    ...
    describe('package.json post-processing', () => {
      it('should set main to remoteEntry.js and types to @mf-types', async () => {
        await command(defaultOpts);
        const pkg = await fs.readJson(joinPath(ctx.targetDir, 'package.json'));
      ...
      it('should remove misleading fields: exports, module, typesVersions', async () => {
        await command(defaultOpts);
        const pkg = await fs.readJson(joinPath(ctx.targetDir, 'package.json'));
      ...
      it('should preserve non-misleading metadata fields', async () => {
        await command(defaultOpts);
        const pkg = await fs.readJson(joinPath(ctx.targetDir, 'package.json'));
      ...
      it('should still apply common bundle post-processing', async () => {
        await command(defaultOpts);
        const pkg = await fs.readJson(joinPath(ctx.targetDir, 'package.json'));
        expect(pkg.bundleDependencies).toBeUndefined();
        expect(pkg.scripts).toEqual({});
    ...
    describe('error handling', () => {
      ...
      it('should not show last 20 lines when verbose=true on failure', async () => {
        ...
        await expect(
          command({ ...defaultOpts, verbose: true }),
