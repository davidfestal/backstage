...
export async function command(opts: OptionValues): Promise<void> {
  ...
  const bundleName = opts.outputName
    ? opts.outputName
    : opts.outputDestination
    ? mangledName
    : 'bundle';
  ...
  try {
    await run(
      ['yarn', 'install', '--no-immutable', '--mode', 'update-lockfile'],
      {
        ...
        onStdout: pruneLog.logRunOutput,
        onStderr: pruneLog.logRunOutput,
      },
    ).waitForExit();
  } catch (err) {
...
async function installBundleDependencies(
  ...
  try {
    await run(['yarn', 'install', '--immutable'], {
      cwd: targetDir,
      onStdout: installLog.logRunOutput,
      onStderr: installLog.logRunOutput,
    }).waitForExit();
  } catch (err) {
...
function createStepLogger(
  ...
  const logRunOutput = (data: Buffer) => {
    if (prefix) {
      for (const line of data.toString('utf8').split(/\r?\n/)) {
        if (line) writeLine(line, 'out');
      }
    } else {
      logStream.write(stripAnsi(data.toString('utf8')));
      if (verbose) process.stdout.write(chalk.dim(data.toString('utf8')));
    }
  };
